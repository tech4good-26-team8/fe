...
export function RecordSheet({ onCancel, onSend }: RecordSheetProps) {
  ...
  return (
    <div className="absolute inset-0 bg-ink/40 flex items-end justify-center z-20">
      <div className="relative w-full max-w-[430px] bg-surface rounded-t-3xl px-8 pt-8 pb-10 flex flex-col items-center gap-6 shadow-2xl">
        <button
          onClick={handleCancel}
          aria-label="취소"
          className="absolute top-4 right-4 w-9 h-9 rounded-full bg-cream border border-border flex items-center justify-center text-ink-muted"
        >
        ...
        <button
          ...
          className={`w-20 h-20 rounded-full bg-accent flex items-center justify-center text-white disabled:opacity-50 ${
